...
func (app *application) readIDParm(r *http.Request) (int64, error) {
...
func (app *application) readJSON(w http.ResponseWriter, r *http.Request, dst interface{}) error {
	//limiting size to 1mb
	maxBytes := 1_048_576
	r.Body = http.MaxBytesReader(w, r.Body, int64(maxBytes))
	//Initializing the json.Decoder and calling the DisallowUnkownFields() method on it before decoding.
	//if json from the client inculdes any field which can be mapped to target destination,the decoder will return an error instead of just ignoring the field
	...
	if err != nil {
		...
		//we trim the preffix and put our format of error
